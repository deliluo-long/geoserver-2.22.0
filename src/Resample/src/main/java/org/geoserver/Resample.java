...
@DescribeProcess(title = "Resample", description = "重采样")
public class Resample implements GeoServerProcess {
    public Resample() {}

    @DescribeResult(name = "outputGridCoverage", description = "输出重采样结果")
    public GridCoverage2D execute(
            @DescribeParameter(name = "inputGridCoverage", description = "输入待重采样影像")
                    GridCoverage2D inputGridCoverage,
            @DescribeParameter(name = "targetGridCoverage", description = "输入参照标准影像")
                    GridCoverage2D targetGridCoverage)
            throws Exception {
        try {
            // 获取源和目标投影的坐标参考系统
