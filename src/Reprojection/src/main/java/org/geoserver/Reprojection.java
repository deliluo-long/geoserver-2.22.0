...
@DescribeProcess(title = "Reprojection", description = "重投影")
public class Reprojection implements GeoServerProcess {
    public Reprojection() {}

    @DescribeResult(name = "outputGridCoverage", description = "输出重投影结果")
    public GridCoverage2D execute(
            @DescribeParameter(name = "inputGridCoverage", description = "输入待重投影影像")
                    GridCoverage2D inputGridCoverage,
            @DescribeParameter(name = "targetCRS", description = "输入目标投影CRS") String targetCRS)
            throws Exception {
        try {
            // 获取源和目标投影的坐标参考系统
            ...
            GridCoverage2D reprojectedCoverage =
                    (GridCoverage2D) Operations.DEFAULT.resample(inputGridCoverage, targetCRSObj);
