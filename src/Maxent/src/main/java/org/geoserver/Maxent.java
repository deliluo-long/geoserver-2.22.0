...
@DescribeProcess(title="Maxent", description="最大熵预测潜在分布")
public class Maxent implements GeoServerProcess {
    public Maxent() {
    }
    ...
    @DescribeResult(name="outputGridCoverage", description="输出最大熵模型潜在分布图")
    public GridCoverage2D execute(       @DescribeParameter(name = "inputFeatures",description = "输入活动点位数据",max = 1,min = 1) SimpleFeatureCollection inputFeatures,
                                 ...
                                 @DescribeParameter(name = "inputRasterName9",description = "输入对应字段名",max = 1,min = 0) String rasterName9) throws IOException, InterruptedException {
        String folderPath = "./src/main/webapp/data/python/Maxent/raster";
        ...
        String species=convertToCSV(inputFeatures,"./src/main/webapp/data/python/Maxent/inputFeatures.csv");
        convertToAsc(raster1, "./src/main/webapp/data/python/Maxent/raster/"+rasterName1+".asc");
        List<String> rasterNameList = new ArrayList();
        if (raster2 != null) {
            convertToAsc(raster2, "./src/main/webapp/data/python/Maxent/raster/"+rasterName2+".asc");
        }
        if (raster3 != null) {
            convertToAsc(raster3, "./src/main/webapp/data/python/Maxent/raster/"+rasterName3+".asc");

        }

        if (raster4 != null) {
            convertToAsc(raster4, "./src/main/webapp/data/python/Maxent/raster/"+rasterName4+".asc");

        }
        if (raster5 != null) {
            convertToAsc(raster5, "./src/main/webapp/data/python/Maxent/raster/"+rasterName5+".asc");

        }

        if (raster6 != null) {
            convertToAsc(raster6, "./src/main/webapp/data/python/Maxent/raster/"+rasterName6+".asc");

        }

        if (raster7 != null) {
            convertToAsc(raster7, "./src/main/webapp/data/python/Maxent/raster/"+rasterName7+".asc");

        }
        if (raster8 != null) {
            convertToAsc(raster8, "./src/main/webapp/data/python/Maxent/raster/"+rasterName8+".asc");

        }

        if (raster9 != null) {
            convertToAsc(raster9, "./src/main/webapp/data/python/Maxent/raster/"+rasterName9+".asc");

        }
        String sourcePath = "./src/main/webapp/data/python/Maxent/raster/"+rasterName1+".prj";
        String destinationPath = "./src/main/webapp/data/python/Maxent/result/"+species+".prj";
        ...
        String outputdirectory = "./src/main/webapp/data/python/Maxent/result";
        String samplesfile = "./src/main/webapp/data/python/Maxent/inputFeatures.csv";
        String environmentallayers = "./src/main/webapp/data/python/Maxent/raster";
        ...
        try {
            ...
            if (exitValue == 0) {
                ...
                if(Files.exists(destination)) {
                    File file = new File("./src/main/webapp/data/python/Maxent/result/"+species+".asc");
                    ArcGridReader reader = new ArcGridReader(file);
                    GridCoverage2D ouputGridCoverage = reader.read(null);
                    // 关闭资源
                    reader.dispose();
                    // 删除文件
                    if (file.exists()) {
